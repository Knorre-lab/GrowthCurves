...
#### Define size of sliding window for fitting (the number of experimental values)
#### Take smaller window for fast-growing organisms
window <- 50

## IMPORTANT! you have to set up the time interval manually in the code
## The default value is 5 minutes
time <- 5 #### in minutes



# Set-up headings to output file
...
write (ColNamesOut, file = "report.txt", append = TRUE)

## Read table - CHECK SEPARATORS!
## Add column with time intervals
inn <- read.table("sample_growthData.csv", header=TRUE, sep =";")
incubation_time <-length(inn[,1])
time_column <- time*c(0:(incubation_time-1)) # set time scale here

# Merging data & calculated log OD
inn <- cbind(time_column, inn)
innlog2 <- log2(inn)
...
for(i in c(first_measurement:(row_number-window)))  
    {
...
tidy_output <- paste(mu, 
                     ...
                     mu_14,
                     sep = ";")
write (tidy_output, file = "report.txt", append = TRUE)
}
